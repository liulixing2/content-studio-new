...
- DeepSeek 返回正文粘贴导入，并自动套公众号内联富文本模板。
- 本地发布前质检。
- 未保存临时结果离开页面提示。
- 手动热点粘贴整理。
- 临时草稿和作品库文章导出 Word。
...
frontend/
  ...
  src/views/articles/           # 公众号页面
    ...
    components/                 # 页面局部组件
      ...
      ManualAiPanel.vue         # 正文 Prompt 组装、复制和粘贴导入
...
生成标题
  ↓
生成临时草稿 / 手动组装 DeepSeek 正文 Prompt
  ↓
...
- 右侧工具箱：放选题设置、生成正文 Prompt 和作品库。

这样页面主线是“当前这篇稿子能不能发布”，工具区只负责提供关键词配置、正文 Prompt 组装和历史文章。
...
生成正文 Prompt 面板以一个可编辑大输入框为核心。

可插入模块：

- 基础规则：固定输出格式和禁止项。
- 方向 / 分类：插入当前关键词、已选方向、读者问题和写作角度。
- 标题：插入当前标题；没有标题时插入可手动填写的占位文本。
- 热词：插入用户手动填写的热词、补充要求或禁用表达。
- 大纲：插入通用公众号正文结构。
- 模板：插入几种固定正文结构模板，例如情绪共鸣文、单作品深聊、对比观点文、清单盘点文、普通观点文。

用户可以只插入一个模块，也可以插入多个模块。最终复制给 DeepSeek 的内容永远以大输入框为准，项目不会自动调用 DeepSeek，也不会保存 Prompt。

## 公众号模板导入

粘贴导入会做确定性清洗和排版：

- 识别 `标题：`、`摘要：`、`互动引导：`、`素材说明：` 等常见标签。
- 识别 `一、`、`1.`、`##` 等小标题。
- 如果没有小标题，会按段落自动拆成开头、正文、结尾。
- 复制富文本使用内联样式，降低粘贴到公众号编辑器后样式丢失的概率。
- 配图建议不再混入正文复制内容，避免误发布编辑提示。
...
- 正文 Prompt 手动补充：用户可以在大输入框里自行添加热词、平台讨论或其他要求。
