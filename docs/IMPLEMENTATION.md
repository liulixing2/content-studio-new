...
- 右侧工具箱：放选题设置、方向来源、DeepSeek 助手和作品库。

这样页面主线是“当前这篇稿子能不能发布”，工具区只负责提供素材、方向、Prompt 和历史文章。
...
- 标题：生成标题候选。
- 大纲：可选，复杂文章再使用。
- 正文：可按标题直接生成，也可基于大纲生成。
- 配图：正文稳定后再生成配图方案。

这些步骤只是生成可复制 Prompt，不自动调用 DeepSeek，不自动保存结果。
...
- 平台/DeepSeek 粘贴生成方向：用户从百度、微博、B站或 DeepSeek 复制内容，粘贴回来整理成临时方向。
